package nl.wlagemaat.demo.exe.workflow;
...
import nl.wlagemaat.demo.exe.workflow.execution.activity.SendToExecutionActivityImpl;
...
import static nl.wlagemaat.demo.clients.InsuranceCaseWorkflow.PRE_INTAKE_NAMESPACE;
...
public class TemporalService implements InitializingBean {
    ...
    public void afterPropertiesSet() {
        ...
        Worker worker = factory.newWorker(INSURANCE_CASE_TASK_QUEUE);

        // Specify which Workflow implementations this Worker will support
        worker.registerActivitiesImplementations(new SendToExecutionActivityImpl());

        // Begin running the Worker
        factory.start();

        log.info("{} started for task queue: {}", worker.getClass().getName(), INSURANCE_CASE_TASK_QUEUE);
    }
